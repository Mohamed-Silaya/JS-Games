document.addEventListener("DOMContentLoaded", () => {
  ...
  function checkGameEnd() {
    if (attempts.correct === Number.parseInt(selectCellsNumber.value) / 2) {
      ...
      // Save score
      var playerName = localStorage.getItem("currentPlayerName") || "Anonymous";
      var score = {
        name: playerName,
  ...
  function showHighScores() {
    // Create scores modal
    const scoresModel = document.createElement("div");
    scoresModel.className = "scores-model";
    scoresModel.innerHTML = `
      <div class="scores-content">
        <h2 class="scores-heading">High Scores</h2>
    ...
    const scoresList = scoresModel.querySelector(".scores-list");
    ...
    for (let i = 0; i < scores.length; i++) {
      ...
      scoreElement.innerHTML = `
        ...
        <span>${score.gridSize}</span>
        <span>${score.time}</span>
        <span>${score.wrongAttempts}</span>
    ...
    // Show the modal
    scoresModel.style.visibility = "visible";
    scoresModel.style.opacity = "1";
